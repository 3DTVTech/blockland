class Game{
	constructor(){
		...
		const game = this;
		this.anims = ['Walking', 'Walking Backwards', 'Turning', 'Run', 'Pointing', 'Talking', 'Pointing Gesture'];
		...
		document.getElementById("animation-btn").onclick = function(){ game.animation(); };
		document.getElementById("profile").onclick = function(){ game.profile(); };
	...
	init() {
		...
		const shadowSize = 40000;
		light = new THREE.DirectionalLight( 0xffffff );
		light.position.set( 0, 6000, 100 );
		...
		// light.shadow.bias = 1;
        light.shadow.mapSize.width = 90000000;
        light.shadow.mapSize.height = 90000000;
		this.scene.add( light );
		...
		// const loaderScenes = new THREE.GLTFLoader();
		// const loaderScenes = new GLTFLoader()
		const game = this;
	...
	playerControl(forward, turn){
		...
		if (forward>0.3){
			if (this.player.action!='Walking' && this.player.action!='Run') this.player.action = 'Walking';
		}else if (forward<-0.3){
...
class Player{
	constructor(game, options){
		...
		loader.load( `${game.assetsPath}fbx/people/${model}.fbx`, function ( object ) {
			...
			player.object.position.set(1, 10, 10000);
			player.object.rotation.set(0, 2.6, 0);
			// player.object.scale.set(2, 2, 2);
